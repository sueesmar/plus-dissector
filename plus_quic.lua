...
-- Get the next Layer for dissection (example quic, which uses udp port 443)
quic_proto = DissectorTable.get("udp.port"):get_dissector(443)
...
--local pf_data = ProtoField.new("Data", "plus.data", ftypes.BYTES, nil, base.NONE, nil, "Data of higher layer") --data not needed, because this ist handled by next layer
...
function plus_proto.dissector(tvbuf,pinfo,root_tree)
    ...
    -- Set the length of the PLUS header to only use this and give the rest to the next layer;
    -- Problem to get the length, because field access only possible later. How to get this????
    -- local plus_header_len = PLUS_BASIC_HEADER_LEN
    ...
    if extended_field()() then
       ...
       -- call the next dissector and give it the rest of the packet
        quic_proto:call(tvbuf:range(22+length_field()(),pktlen-(PLUS_EXTENDED_HEADER_LEN_MIN+length_field()())):tvb(),pinfo,root_tree)
    else
        -- basic header, so print data
        --plus_tree:add(pf_data, tvbuf:range(20,pktlen-PLUS_BASIC_HEADER_LEN))
        -- call the next dissector and give it the rest of the packet
        quic_proto:call(tvbuf:range(20,pktlen-PLUS_BASIC_HEADER_LEN):tvb(),pinfo,root_tree)
    end
end
